// Score3 위젯은 초기값과 번호를 이용하여 API 요청을 보내고 결과를 처리하며, UI를 업데이트합니다.
// fetchdata 함수는 API 요청을 처리하고 결과를 화면에 반영하며 저장합니다.
// fetchavg 함수는 평균 결과를 가져오는 API 요청을 처리합니다.

import 'package:bridze/chart/chart_language_8.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';

class Score3 extends StatefulWidget {
  final String initialValue;
  final int number;

  // 생성자로 받아온 초기값에 따라 crr로 비교할 글이 정해집니다.
  const Score3({Key? key, required this.initialValue, required this.number})
      : super(key: key);

  @override
  Score3State createState() => Score3State();
}

class Score3State extends State<Score3> {
  late String value;
