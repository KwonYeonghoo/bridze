...
- Media pipe (CV 모델)
  : Vggnet 모델을 통해 표정을 분류하기 전에 google의 Media pipe 기술로 얼굴 영역을 인식합니다. Media pipe 관련 코드는 를 참고하시면 됩니다.

- Vggnet (CV 모델)
: 기존 Vggnet 모델은 외국인의 얼굴을 잘 인식하고 표정을 잘 분류했지만 비교적 이목구비가 뚜렷하지 않은 동양인의 얼굴은 잘 인식하지 못하는 모습을 보였습니다. 다문화가정 아동들은 한국인 부모님과 타국가의 부모님 사이에서 태어난 아동들이기에 Vggnet 모델이 동양인의 얼굴과 표정 역시 잘 인식할 수 있도록 추가학습을 진행하였습니다. 해당 모델은 BridZe 내 정서 평가의 표정 평가 및 분류에서 활용됩니다.
...
|ai hub 데이터셋 vggnet (6만 ver)|77.1|
